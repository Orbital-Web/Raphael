...
struct v1_8_params {
    ...
    const int PV_STABLE_COUNT = 6;     // number of consecutive bestmoves to consider pv as stable
    const int MIN_SKIP_EVAL = 200;     // minimum eval to halt early if pv is stable
    ...
    const int KING_DIST_WEIGHT[2] = {0, 20};  // weight scaling for closer kings
    const int DRAW_DIVIDE_SCALE = 32;  // how much to scale evaluation by for likely draw (e.g. KNN)
    const int PVAL[12][2] = {
        // WHITE
        {100,   100 }, // PAWN
        {418,   246 }, // KNIGHT
    ...
    const int PAWN_PASSED_WEIGHT[7][2] = {
        {0,   0  }, // promotion line
        {114, 215},
        {10,  160},
        {4,   77 },
    ...
    const int PAWN_ISOLATION_WEIGHT[2] = {29, 21};  // cost of having an isolated pawn
    const int MOBILITY_BISHOP[2] = {12, 6};         // bonus for each square the bishop sees/xrays
    const int MOBILITY_ROOK[2] = {11, 3};           // bonus for each square the rook sees/xrays
    const int BISH_PAIR_WEIGHT[2] = {39, 72};       // bonus for having a bishop pair
    const int BISH_CORNER_WEIGHT[2] = {1, 20};  // bonus for enemy king distance to bishop corner
